...
impl From<Votingmachine> for VotingmachineDao {
	fn from(machine: Votingmachine) -> Self {
		...
		let voters_who_voted = machine
			.get_voters_who_voted()
			.iter()
			.map(|voter| voter.0.clone())
			.collect();
...
impl From<VotingmachineDao> for Votingmachine {
	fn from(machine_dao: VotingmachineDao) -> Self {
		...
		let voters_who_voted = machine_dao
			.voters_who_voted
			.into_iter()
			.map(Voter)
			.collect::<Set<Voter>>();
		...
		let valid_candidates = scoreboard.scores.keys().cloned().collect();
		Votingmachine::from_parts(voters, voters_who_voted, scoreboard, valid_candidates)
	}
}
