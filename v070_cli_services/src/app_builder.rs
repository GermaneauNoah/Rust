...
pub async fn handle_lines<Store: Strorage + Send + Sync, Serv: Service<Store>>(configuration: Configuration) -> anyhow::Result<()> {
    ...
    Serv::new(port, lexicon, controller).serv().await
}
...
async fn dispatch_service<Store : Strorage + Send + Sync>(conffiguration: Configuration) -> Result<(), anyhow::Error> {
    match conffiguration.service() {
        ServiceType::Stdio => handle_lines::<Store, StdioService<Store>>(conffiguration).await,
        ServiceType::Udp => handle_lines::<Store, UdpService<Store>>(conffiguration).await,
    }
}
