use crate::domain::{AttendencesSheet, Scoreboard};
...
pub async fn handle_line<Store: Strorage>(
    ...
    match cmd {
        "voter" | "vote" => {
            ...
            Ok(super::show_vote_outcome(outcome, lexicon))
        }
        "votants" | "voters" => {
...
mod tests {
    ...
    async fn test_commande_vide_retourne_inconnue() {
        ...
        let res = handle_line("", &ctrl, &FRENCH).await.unwrap();
    ...
    async fn test_afficher_scores() {
        let ctrl = make_controller().await;
        let res = handle_line("scores", &ctrl, &FRENCH).await.unwrap();
    ...
    async fn test_voter() {
        let ctrl = make_controller().await;
        let res = handle_line("voter alice bob", &ctrl, &FRENCH).await.unwrap();
    ...
    async fn test_voter_blanc() {
        let ctrl = make_controller().await;
        let res = handle_line("voter alice", &ctrl, &FRENCH).await.unwrap();
        assert!(res.contains("alice"));
    ...
    async fn test_voter_sans_nom_demande_votant() {
        let ctrl = make_controller().await;
        let res = handle_line("voter", &ctrl, &FRENCH).await.unwrap();
    ...
    async fn test_commande_invalide() {
        let ctrl = make_controller().await;
        let res = handle_line("xyz", &ctrl, &FRENCH).await.unwrap();
        assert!(res.contains(FRENCH.unknown_command));
