...
impl <Store: Strorage> VotingController<Store> {
    ...
    pub async fn vote(&self, form: VoteForm) -> anyhow::Result<VoteOutcome> {
        let ballot_paper: BallotPaper = form.into();
        let mut store_lock = self.store.write().await;
        let mut voting_machine = store_lock.get_voting_machine().await?;
        let outcome = voting_machine.vote(ballot_paper);
        store_lock.put_voting_machine(voting_machine).await?;
        Ok(outcome)
    }
